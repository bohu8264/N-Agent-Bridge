...
## 0.13.5 已完成（全新 Mac 配置一致性与 HID 事务可靠性）

- 定位跨机器偶发问题的两个软件根因：NuPhy S4 回复没有 transaction ID，多个 IOHID session 并发时可能把同一 ACK/数据帧交给错误事务；0.13.2 的已知异常绑定修复又只在 schema 升级路径运行，已经写成 schema 13 的坏配置会永久绕过。
- 新增进程级 HID 事务协调器，序列化键位、D5/D6 与 D8 管理帧；首次配置期间暂停灯光/状态同步，新枚举或唤醒时键位验真、D5/D8 失败最多自动重试三次，不再依赖用户稍等后手动刷新。
- schema 14 在每次配置加载时对 Air75 V3、Kick75 IO、Node100 LP ANSI 精确匹配并修复 `F13 / F15 / Tab / F16…` 历史序列；新 Mac 的全新配置保持实体 F1–F12，连接并启用后才由安全备份/写入/回读流程切换为 F13–F24。
- 已记录硬件 Profile 通过直接 USB-C 重连时会读取键盘真实 keymap；固件更新重置后会停用控制并要求配置，不再让 F1–F12 同时触发 macOS 原生功能。U1 接收器明确排除在首次配置/恢复路径之外。
- 三个已验证型号首次配置统一选择“指示灯”模式；DMG 增加中文最短安装说明。项目目录清除旧 `.build`、旧 `dist`、`.DS_Store`、空 entitlements 和界面已移除的亮度入口；硬件备份目录未触碰。
- software-only SelfTest 已通过，包含 schema 14 三型号迁移和全新配置默认值回归。Universal（arm64 + x86_64）、固定签名、Bundle 资源、DMG CRC/结构和只读挂载内容验证全部通过；SHA-256：`71d7a873187da979d4eec3e499581221e05d9a811294da9ab9a9acf0d121956e`。当前没有 NuPhy 键盘在线，实体键、灯光和恢复仍需在朋友机安装后分别复核。

## 0.13.4 已完成（Kick75 首次连接灯光待响应修复）

- 修复朋友电脑上“概览正常、Kick75 灯光页却长期显示 USB-C 待响应且全部控件变灰”：0.13.2 为避免首次写键位时争抢 HID 通道，会在当前型号专用层尚未安装时跳过 D5 灯光读取，但概览只检查了全局 Codex 配置，导致换到 Kick75 时把旧型号记录误报成当前型号已经就绪。
- 设备识别后现在允许在硬件配置前先做安全的只读 D5 握手；写入保护和 Kick75 已验证 handle 白名单保持不变。首次配置等待窗口覆盖固件完整重试周期，避免读取刚结束时配置流程提前超时。
- 概览和首次设置现在同时检查“当前连接型号”的硬件 Profile；Kick75 未配置时明确显示“需要配置当前键盘”，按钮执行完整的键盘配置，而不是只修复 Codex 快捷键。
- Debug App、software-only SelfTest、Universal（arm64 + x86_64）固定签名 App、Bundle 资源、DMG CRC/结构验证通过。Development DMG SHA-256：`a1ef87df42aae6643e4113525dcee4cb84a039937b8ae06f18472e40a4c0b978`。本轮没有 Kick75 实机在线，最终 D5/D6 目视验收由安装新版后的朋友电脑完成。
...
- schema 升级到 13，只匹配这组完整 action/usage 历史签名并恢复连续 F13–F24；不会全量重置配置，Q、数字键等真正学习过的自定义绑定保持不变。启用时再做一次同样的精确防护，已损坏配置无需用户逐键重设。
- 首次设备识别不再在尚未安装专用层时自动抢先发起 D5 灯光刷新；“连接并启用”若遇到正在进行的灯光事务会等待其结束，再依次执行键位备份/写入、Codex 快捷键安装和指示灯模式 D6 写入，避免过一会才突然生效。
- 配置完成前“概览”的 Codex 控制状态明确显示“正在配置”并隐藏重复修复按钮；完成后若没有直接取得灯光状态，会立即刷新一次，不再依赖下一次设备事件偶然触发。
- Debug App 与 software-only SelfTest 通过，新增异常序列自动迁移及自定义键保留回归；Universal（arm64 + x86_64）固定签名 App、Bundle 资源、DMG CRC/结构验证通过。Development DMG SHA-256：`83b95375a3dbc8cc6643dff976178a9315194353b39d49cca23b9d9957faeb28`。
