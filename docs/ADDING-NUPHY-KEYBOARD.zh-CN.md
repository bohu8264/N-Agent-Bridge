...
当前 Air65 V3、Air100 V3、Node75 与 Node100 未实测变体已完成安全识别阶段。Profile 的 PID 来自官方 NuPhyIO 设备目录，但没有已验证 driver ID 的型号不会改写键盘固件。Kick75 IO 已注册独立 `nuphy.s4.kick75-keymap`；Node100 LP ANSI（`19F5:1037`）已注册独立 1904-byte keymap、D6、D8 与 sleep driver。两者的矩阵、触控装置和灯位都不能复制给其他型号。
