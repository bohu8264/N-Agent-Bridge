...
当前 Air65 V3、Air100 V3、Node75、Node100 已完成这一阶段。Profile 的 PID 来自官方 NuPhyIO 设备目录，但 `capabilities` 中没有 driver ID，因此用户可以把数字、字母、F 区或导航键映射到 Codex，而应用不会改写键盘固件。Kick75 IO 已继续完成独立实机阶段并注册 `nuphy.s4.kick75-keymap`，不可把它的 1472-byte/92-entry 布局复制给其他型号。
