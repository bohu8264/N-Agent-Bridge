...
当前本机开发版：`0.13.1 (49)`（首次/升级启用会在 USB-C 下重新读取真实键位，自动修复 F1–F12 为 F13–F24，并一次性选择指示灯背光；中断的历史任务不再永久显示蓝灯）。

- 源码根目录：本文件所在目录
- 0.13.0 按 NuPhyIO 官方 Node100 LP ANSI 的 108 键顺序补齐 D8 布局；Q=44 已完成写入、D2 回读和原色恢复实测。
- 当前安装位置：`/Applications/N Agent Bridge.app`，版本 `0.13.1 (49)` 已启动。最终复核时没有 NuPhy 键盘在线，不能把本轮 USB-C 键位/灯光实写记为已验收；下次接入已验证型号后必须点一次“连接并启用”，确认完整键位回读、灯光通道、HID open、输入监控与辅助功能。
